describe("LPeg serialization", function()
  ...
  it('should serialize attached values', function()
    assert.are.same(true,    (L.P'aaa' * L.Cc(true)):match('aaa'))
    assert.are.same(42,      (L.P'aaa' * L.Cc(42)):match('aaa'))
    assert.are.same('foo',   (L.P'aaa' * L.Cc('foo')):match('aaa'))
    assert.are.same({1,2,3}, (L.P'aaa' * L.Cc({1,2,3})):match('aaa'))
    assert.are.same('AAA',   (L.P'aaa' / function(s) return s:upper() end):match('aaa'))
  end)
